    ...
    public void calculateNumMines() {
        ...
        for (int r = 1; r <= numberOfRows; r++) {
            for (char c = 'A'; c <= 'A' +numberOfColumns - 1; c++) {
                minefound:   {
                    for (int i = 0; i < numberOfMines; i++) {
                        if (cvcr.mineLocation[i].column == c //checks if a mine matches current row
                                && cvcr.mineLocation[i].row == r) { //and column.
                    ...
                    for (int k = 0; k < numberOfMines; k++) { //counter for loop to calculate cell value.
                        ...
                        if (c > 'A' && cvcr.mineLocation[k].column == (c-1) // Left one column. Skips if first column
                                && r > 0 && cvcr.mineLocation[k].row == (r-1)) //Up one row. Skips if first row
                            tempCellValue++;
                        //If cell left of current cell is mine, add one to tempCellValue.
                        if (c > 'A' && cvcr.mineLocation[k].column == (c-1) //Left one column. Skips if first column
                                && cvcr.mineLocation [k].row == r)
                            tempCellValue++;
                        //If cell down and to the left of current cell is mine, add one to tempCellValue.
                        if (c > 'A' && cvcr.mineLocation[k].column == (c-1) //Left one column. Skips if first column
                                && r < numberOfRows && cvcr.mineLocation[k].row == (r+1)) //Down one row. Skips if last row
                            tempCellValue++;
                        //If cell directly above current cell is mine, add one to tempCellValue.
                        if (cvcr.mineLocation[k].column == c
                                && r > 0 && cvcr.mineLocation[k].row == (r-1)) //Up one row. Skips if first row
                            tempCellValue++;
                        ...
                        if (c < 'A' + numberOfColumns && cvcr.mineLocation[k].column == (c+1) //Right one column. Skips if last column
                                && r > 0 && cvcr.mineLocation[k].row == (r-1)) //Up one row. Skips if first row
                            tempCellValue++;
